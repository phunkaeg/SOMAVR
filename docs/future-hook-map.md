...
| `Soma_NoSteam.exe+0x0cd710` | Static-world control call built | Registered `CheckLineOfSight` wrapper | `0.24.0` directly calls the guarded wrapper; `0.25.0` reuses it for center/radial/top/bottom head-volume samples. No detour and no script ownership change. |
| `Soma_NoSteam.exe+0x143650` | Static confirmed | Underlying physics/world line query | Active-world resolution and physics callback are documented for future dynamic or shape-query RE; current build uses the wrapper with static-only filtering. |
...
| `Player.hps::UpdateFlashLightLOS` camera-ray sample | Script confirmed, RE required | Align randomized agent-gobo gameplay rays with the moved controller flashlight | General light frustum/sensor behavior already follows the exact light matrix; isolate only the three low-frequency camera pitch/yaw rays after visual acceptance. |
