// somavr_xrsim64 logging and data directory.
//
...
void init() {
    ...
    // SOMAVR_XRSIM_DIR exists so a self-test process and a live game session cannot
    ...
    DWORD n = GetEnvironmentVariableW(L"SOMAVR_XRSIM_DIR", g_dir, MAX_PATH);
    if (n == 0 || n >= MAX_PATH) {
        wchar_t local[MAX_PATH];
        if (FAILED(SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, local))) return;
        swprintf_s(g_dir, L"%s\\SOMAVR", local);
...
void rig_defaults(Rig& r) {
    ...
    //
    // PINNED to this machine's measured Quest 3 / VDXR values: the real-headset
    // log reports "headset fov half-angles h=54.0 v=55.0". The
    // published-figures guess this replaces
