...
| S13 | Dual-render test scenes | In F10 VR, enable `SAME FRAME STEREO` from F1 in quiet, shadowed, reflective, tone/bloom, fade, HUD/menu, and post-heavy views; toggle it off/on and take one Ctrl+F6 sample | Continuous same-pose/opposite-eye exact-player replays, first-eye immediate cache, GUI bit removed, one upper frame lifecycle, bounded logs, diagnostic sample precedence, clean AFR rollback | Crash, extra upper lifecycle, simulation advance, eye/pose mismatch, duplicated GUI, resource overflow, persistent temporal artifact, unacceptable frame pacing, or broken AFR fallback |
