...
| `0x1402f0c90` | Confirmed, control hook and direct dispatch built | `HPL3_ImGui_SendMouseVirtualPosition`; registered virtual-coordinate cursor wrapper and exact world/3D GUI dispatch boundary. It stores position at cImGui `+0x4f3c/+0x4f40`, relative delta at `+0x4f4c/+0x4f50`, and clears first-update flag `+0x4f72`. `0.65.0` calls the original function directly for each accepted controller mesh-ray position because a controller-only session produces no native mouse-motion callback to trigger substitution. Exact terminal states, current non-GameHud ImGui, and readable 3D cGuiSet ownership remain mandatory. |
...
| `0x140238750` | Confirmed, control hook built | `HPL3_PidControllerVec3_Output`. Grab force (`400/0/40`) receives the bounded selected-hit-to-grip correction. `0.65.1` replaces only Grab torque (`40/0/0.4|0.1`) with absolute tracked-hand body target velocity minus measured body angular velocity, eliminating the competing native camera-relative target. Slide `6/0/0.1` and SwingDoor/Lever `10/0/1` retain their joint-pin routes. SOMA still owns PID integration, limits, mass, inertia, collision, joints, gravity, sounds, and callbacks; every unrelated tuple/state passes through. |
