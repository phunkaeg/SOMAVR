...
| `FEATURE.TERMINAL_OVERLAY` | BUILT | `HPLHudBridge`, `HPLTerminalBridge`, `HPLInputBridge`, `OpenXRGLBridge`, `OpenXRRuntime` | state `8`, exact manager world owner `+0x170`, focused wrapper/set `+0x180/+0x18`, flat `1024x577` set, `0x140213970`, additive HUD capture | `FUTURE_SYSTEMS_RE.md`, `ADDRESS_REGISTRY.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test exact terminal duplication, continuous two-hand pointer alignment, native widget clicks, physical-screen preservation, alpha/order, exit cleanup, and `TerminalOverlay=0` rollback |
...
| `FEATURE.PHYSICS_HANDS` | BUILT | `HPLGrabBridge`, `HPLGrabMath`, `HPLTwoHandMath`, `HPLInputBridge`, `OpenXRInput` | PID output `0x140238750`, body matrix `+0x50`, angular velocity vtable `+0x90`, bounded absolute angular error, AddImpulse thunk `0x14049c720`, Grab state `1`, unbounded initial hit-to-grip pull plus bounded post-acquisition travel, state-lifetime owner lock | `VR_COMPATIBILITY_RE.md`, `FUTURE_SYSTEMS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test mug/DSLR full pull-in and 1.5 m controller travel, jitter-free one/two-hand rotation, hold stability, placement, throws, crossed-beam ownership, mass classes, and fallbacks |
| `FEATURE.PHYSICAL_MANIPULATION` | BUILT | `HPLGrabBridge`, `HPLGrabMath`, `HPLReadMath`, `HPLInputBridge`, `HPLInputMath`, `HPLInteractionBridge`, `OpenXRInput` | states Wheel `3`, Slide `4`, Door `5`, Lever `6`, Tear `7`, Read `10`; native Slide `OnAnalogInput(Look)` to `mvMoveAdd`; optional PID `0x140238750`, pin `+0xe8`, pivot `+0xf4`; native Read travel plus persistent full-axis grip orientation | `FUTURE_SYSTEMS_RE.md`, `ADDRESS_REGISTRY.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test curtain/drawer native drag response without snap turn, wrist-driven taps/levers, Read cancel/camera stability; preserve native limits, physics, sounds, callbacks, and rollback |
