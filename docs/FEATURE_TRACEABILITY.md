...
| `FEATURE.PHYSICAL_MANIPULATION` | BUILT | `HPLGrabBridge`, `HPLGrabMath`, `HPLReadMath`, `HPLInputBridge`, `HPLInputMath`, `HPLInteractionBridge`, `OpenXRInput` | Wheel `3`, Slide `4`, Door `5`, Lever `6`, Tear `7`, Read `10`, MovingButton `13`; queued Look plus guarded native-input-phase substitution at `0x140154fb0`; current-native Read scaling/full-axis orientation | `FUTURE_SYSTEMS_RE.md`, `ADDRESS_REGISTRY.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | First prove curtain entry/drag no longer faults and native dispatch counters rise; then test tap/cupboard direction changes and Read stability while preserving native limits, sounds, callbacks, and rollback |
