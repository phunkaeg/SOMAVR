...
| `FEATURE.XR_DEPTH_CAPABILITY` | EXPERIMENTAL | `OpenXRRuntime`, `OpenXRGLBridge`, `HPLCameraBridge` | `XR_KHR_composition_layer_depth`, per-eye GL depth caches, `GL_DEPTH_BITS`, sampled source depth, HPL near/far | `FUTURE_SYSTEMS_RE.md`, `VR_COMPATIBILITY_RE.md`, `TEST_CHECKLISTS.md` | Live logs prove valid, varying depth for both AFR eyes before depth swapchains and `XrCompositionLayerDepthInfoKHR` submission |
