...
| `0x1400bcd90` | Confirmed, control hook built | Shared AngelScript `iLuxEntity.SetMatrix` registration target used by derived Lux entity types. It owns exact independent `HudObject`, socketed `*_HudObject` classification, `PlayerHands_*`, and `Flashlight` policies. `0.65.0` additionally recognizes only nearby non-special entities during exact Read state `10`, caches first camera distance per entity, and applies the configured scale/distance transform; destruction evicts that cache. Every failed identity, state, distance, tracking, or matrix gate forwards the original pointer. Ghidra: `SOMA_iLuxEntity_SetMatrix`. |
...
| `0x140238750` | Confirmed, control hook built | `HPL3_PidControllerVec3_Output`. Grab force (`400/0/40`) and torque (`40/0/0.4|0.1`) retain their controller targets. `0.65.0` adds a bounded selected-hit-to-grip correction at Grab anchor, then tracked controller deltas. Slide state `4` force PID `6/0/0.1` receives controller velocity projected onto the native pin. SwingDoor/Lever states `5/6`, only at torque PID `10/0/1`, combine controller arc velocity with grip angular velocity projected onto the native pin. SOMA still owns PID integration, limits, mass, inertia, collision, joints, gravity, sounds, and callbacks; every unrelated tuple/state passes through. |
