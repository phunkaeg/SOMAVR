...
| `FEATURE.XR_GL_SUBMISSION` | PROVEN | `OpenXRRuntime`, `OpenXRGLBridge`, `OpenGLOwnership` | OpenGL swapchains/FBOs, full-state own-GL transaction and detour bypass, one prioritized projection on every begun `xrEndFrame`, per-eye acquire/wait/copy/flush/release timing and display-budget pressure | `CURRENT_STATE.md`, `BUILD_HISTORY.md`, `OPENXR_GL_TRANSFER_RE.md` | Run the controlled VirtualDesktopXR/SteamVR A/B and compare `copyCpu`, not wait-inclusive `projectionUs`; retain native GL unless measured copy cost materially justifies a D3D11 interop backend |
...
| `FEATURE.XR_FOCUS_PACING` | BUILT | `OpenXRRuntime`, `OpenXRFramePacingMath` | session-state events, ever-focused latch, bounded `xrWaitFrame` telemetry, frame-open invariant, stale-cache invalidation on focus recovery | `BIOSHOCK_VR_TRANSFER_AUDIT.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | After first focus, leave the runtime VISIBLE for 60 seconds and prove the desktop game continues; focus restoration resumes stereo/input without F10 |
...
| `FEATURE.TRACKING_RESILIENCE` | BUILT | `OpenXRRuntime`, `HPLCameraBridge` | one-period-ahead render pose, current-pose fallback counter, pose-age bound, retained/black projection, recovery blackout | `BUILD_HISTORY.md`, `CURRENT_STATE.md`, `TEST_CHECKLISTS.md` | Live-test brief and extended HMD tracking loss without stale-eye corruption, stereo teardown, visible recovery flash, or any zero-layer submit |
...
| `FEATURE.DIEGETIC_GUI_POINTER` | BUILT | `HPLTerminalBridge`, `HPLComfortBridge`, `HPLInputBridge`, `HPLPlayerState` | wall/handheld terminal states `8/9`, `0x1403132d0`, direct original `0x1402f0c90` dispatch, `0x140237920`, `0x1401562e0`, `0x140159360`, focused wrapper/entity `+0x180/+0x28`, multi-frame native cancel hold | `ADDRESS_REGISTRY.md`, `FUTURE_SYSTEMS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test right-A and look-away state exit latency, physical lean, pointer/click behavior, state 9 preservation, and all rollback levels |
| `FEATURE.TERMINAL_OVERLAY` | BUILT | `HPLHudBridge`, `HPLTerminalBridge`, `HPLTerminalMath`, `HPLInputBridge`, `OpenGLHooks`, `OpenXRGLBridge`, `OpenXRRuntime` | state `8`, manager world owner `+0x170`, focused set `+0x180/+0x18`, logical `1024x577`/`880x560`, one `0x140213970` render into retained native-size target, impossible-scissor bypass, upscale to HUD, HMD look-away native cancel, exact ray intersection against the configured VIEW-space quad/cylinder, 1.75x pointer | `FUTURE_SYSTEMS_RE.md`, `ADDRESS_REGISTRY.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Headset-accept pointer/GUI coordinate agreement at center and edges, clicks, page changes, look-away exit, state-9 preservation, cleanup, and capture cost |
...
| `FEATURE.AFR_STEREO` | PROVEN | `HPLCameraBridge`, `OpenXRRuntime`, `OpenXRGLBridge` | F11, per-eye cache and submitted render pose, fill-committed phase, pair-latched rotation with per-eye position, transient apply retry with eight-failure suspension, 12-frame last-pair hold using its render poses | `BUILD_HISTORY.md`, `RUNTIME_ANALYSIS_0.5.1.md`, `REVIEW_HANDOVER_2026-08-07.md` | Headset-prove fill commits alternate 0/1 through loading/tracking interruption, transient failures self-recover, held pairs remain world-locked, and pair rotation stays rigid without positional-baseline collapse |
...
| `FEATURE.NATIVE_STEREO` | DESIGNED | future `HPLNativeStereo`, `HPLDualRenderControl`, `HPLCompatibilityProbe` | viewport `0x140298850`, render-list `0x1402975e0`, culling `0x140327570`, occlusion submit `0x1402b3c30`, render-section boundary `0x140298630`, presentation `0x1402a0590` | `NATIVE_STEREO_FEASIBILITY.md`, `ADDRESS_REGISTRY.md`, `VR_COMPATIBILITY_RE.md` | Instrument occlusion-query reuse first, then run a default-off one-second second-world-render experiment with CPU/GPU budget, culling, shadows/reflections, temporal ownership, HUD and rollback evidence |
...
| `FEATURE.LOCOMOTION` | BUILT | `OpenXRInput`, `HPLInputBridge`, `HPLInputMath`, `HPLNativeLocomotion`, `HPLPhysicalCrouchMath`, `HPLPlayerState` | live-proven helper owner root `+0x110`, semantic Move type `1` at `0x140154fb0`, guarded body Move `0x1402375f0` during physical states `3..7/13`, native turn `0x140237460`, calibrated movement-controller direction, delayed yaw-only virtual torso follow without camera mutation | `FUTURE_SYSTEMS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-confirm normal and interaction-state walking speed, controller-relative direction, snap-turn continuity, and 45-degree delayed shoulder follow with a rigid HMD view |
...
| `FEATURE.VISIBLE_HANDS` | BUILT | `HPLHandsBridge`, `HPLHandsMath`, `HPLArmIKMath`, `HPLInputBridge` | exact `PlayerHands_*` identity, validated shared `j_Root` plus bilateral 34-node restore, HMD-position anchor, virtual torso yaw that follows native explicit turns but not HMD camera, geometric palm calibration plus configurable local pitch/world offsets, reach-gated clavicles, ergonomic elbow swivel, native-seeded active/visible wake and retention, physical states `0..8`, Read `10`, and MovingButton `13` | `AUTHORED_STATES_AND_VISIBLE_HANDS_RE.md`, `FUTURE_SYSTEMS_RE.md`, cross-engine `12-torso-calculations-and-ergonomics.md` | Accept wrist calibration, Read/terminal continuity, startup wake, and camera-independent torso follow; resolve/call handler `CreateHandModelIfNeeded` only for maps with no native seed |
...
| `FEATURE.TERMINAL_LAYER_DIAGNOSTICS` | BUILT | `HPLHudBridge`, `OpenXRRuntime`, `OpenXRGLBridge`, `OpenGLHooks` | focused state-8 GUI, `0x140213970`, pre-compositor HUD capture FBO, automatic GL state trace, per-draw zero-intersection scissor bypass with immediate restoration, `Ctrl+F10` RGB/alpha readback | `FUTURE_SYSTEMS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Headset-test complete email rendering and require bounded bypass rows only for impossible offscreen clips; retain valid clipping and compare capture cost |
