...
An RVA in this registry is evidence for the named executable build, not the
runtime authority. Source signatures are authoritative: every code hook or
...
| `0x1402cb5a0` | Confirmed, documented; not hooked | **Sub-mesh level `SetVisible(bool)` --- takes a `cSubMeshEntity`, NOT a `cMeshEntity`.** This is the function `cMeshEntity::SetVisible` invokes on each of its sub-meshes. Writes `+0x38`, dispatches vtable slot `+0xd0`, then dereferences `+8`. Requires a 27+ slot vtable and a valid-or-null pointer at `+8`. Passing a `cMeshEntity` (14 slots) or an `iLuxEntity` crashes --- see [CRASH_F10_DUAL_RENDER_2026-09-01.md](CRASH_F10_DUAL_RENDER_2026-09-01.md). Version 0.95.3 removed the residual wrong-type hook, signature, trampoline, and counters. Ghidra: `HPL3_SubMeshEntity_SetVisible`. |
| `0x1401fe390` | Confirmed by decompilation; intentionally not hooked | **`cMeshEntity::SetVisible(bool)` --- the correct entry point for a `cMeshEntity*`** (the pointer `GetMeshEntity` returns). Guards on `+0x168`, then iterates the child vector `[+0x1f0, +0x1f8)` and calls `0x1402cb5a0` on each child. **Confirmed against HPL2 engine source** (`D:\Dev Debug\source code\AmnesiaAMachineForPigs\HPL2\core\sources\scene\MeshEntity.cpp:1345`), which is the same function line for line:
> `void cMeshEntity::SetVisible(bool abX) { if(abX == mbIsVisible) return; mbIsVisible = abX; for(int i=0;i<(int)mvSubMeshes.size();i++) mvSubMeshes[i]->SetVisible(abX); }`

So `+0x168` is `mbIsVisible`, `[+0x1f0,+0x1f8)` is `mvSubMeshes`, and the children are `cSubMeshEntity` (`: iRenderable : iEntity3D`) --- a deeper hierarchy than `cMeshEntity` (`: iEntity3D`), which is why the child vtable reaches slot `0xd0` and the parent's, at 14 slots, does not. It remains a valid future mesh-level lane, but current retained-hands wake uses only the proven `iLuxEntity::SetActive` owner; live `wake=1` evidence showed no need to add another visibility hook. |
| `0x1401fe1a0` | Confirmed by HPL3 decompilation and HPL2 source; read-only data consumed | `cMeshEntity::UpdateGraphicsForFrame(float)`. Updates each bone state, composes the mesh inverse-world transform with the bone world transform and inverse bind matrix, and writes one 64-byte matrix per bone to `cMeshEntity+0x340`. The bone-state vector is `[+0x2a0,+0x2a8)` and the output-palette vector is `[+0x340,+0x348,+0x350)`. Version 0.95.8 safely snapshots the validated output vector at the existing viewport witness; it does not hook this hot path. Ghidra: `HPL3_MeshEntity_UpdateGraphicsForFrame`. |
| `0x140338900` | Confirmed by HPL3 decompilation and HPL2 source; documented, not hooked | `cSubMeshEntity::UpdateGraphicsForFrame(float)`. Calls the owning mesh update, consumes bone indices/weights plus the palette at mesh `+0x340`, CPU-skins positions, normals, and tangents, then uploads the dynamic vertex buffer. This is the first confirmed owner downstream of the 0.95.7 arm-node witness and the next rung if palettes remain coherent while the eyes deform differently. Ghidra: `HPL3_SubMeshEntity_UpdateGraphicsForFrame`. |
...
| `0x140154fb0` | Confirmed, guarded control call built | Semantic player analog dispatcher. Live Frida on 2026-07-20 proved `this` is the player-helper subobject at root `+0x110`, not the root snapshot pointer. It dispatches to helper `+0xc8` (root `+0x1d8`), secondary helper `+0xf0`, then player handling. Physical mouse Look is type `0`; physical `W` is type `1` amount `{0,1,0}`. Version 0.71 validates root identity/state but calls this address with the helper owner only from native helper update `0x14015ba20`. Arbitrary-phase calls remain prohibited by the 0.68 crash. Ghidra: `SOMA_cLuxPlayer_OnAnalogInput`. |
| `0x140164910` | Confirmed script dispatcher | Calls a player-state script object's boolean `OnAnalogInput(int,const cVector3f&)`. Establishes the native route used by Slide and MovingButton scripts. Ghidra: `SOMA_ScriptPlayerState_OnAnalogInput`. |
| `0x140299fb0` / fault `0x140299fca` | Confirmed crash owner, documented | Selects an available script execution context and reads context-manager vectors at `+0x90`. The 0.68 curtain dump faulted here with a null context manager after out-of-phase direct analog dispatch. Not hooked; native input-phase ownership is mandatory. |
...
| `0x140213970` | Confirmed, control hook built | Renders one `cGuiSet`; 2D sets draw into the current framebuffer. The `0.72.0` dump proved retained terminal rectangles; the `0.73.0` log proved main-HUD retention alone did not stop nested email flashing. `0.74.0` renders the focused state-8 set once into a retained target matching exact logical `+0x100/+0x104` dimensions, then upscales the completed surface to the OpenXR HUD. Replaying this stateful function remains prohibited. Ghidra: `HPL3_GuiSet_Render`. |
...
| `0x1402f0c90` | Confirmed, control hook and direct dispatch built | `HPL3_ImGui_SendMouseVirtualPosition`; stores position at cImGui `+0x4f3c/+0x4f40`, relative delta at `+0x4f4c/+0x4f50`, and clears first-update flag `+0x4f72`. Terminal world input owner is manager `+0x170`, with focused wrapper `+0x180`, set `+0x18`, entity `+0x28`, and screen-input flag `+0x192==0`. Direct dispatch and hooked native updates target this exact owner. `0.67.0` uses physical mesh UVs when the terminal overlay is disabled and stable head-cone/panel coordinates when it is enabled. |
| `0x1400f7f10` | Confirmed | `SOMA_ImGuiManager_UpdateInput`; selects the active ImGui owner. Its world-GUI branch projects native input and calls `0x1402f0c90`; the ordinary screen owner receives `0x1402f0b10`. `0.67.0` retains this dispatch owner while decoupling overlay aim from physical-mesh misses. |
| `0x1403132d0` | Confirmed, consumed | `HPL3_GuiSetEntity_ProjectRayToVirtual`; intersects a world ray segment with the spatial GUI mesh, barycentrically interpolates UV set `7`, and scales by cGuiSet virtual dimensions at `+0x100/+0x104`. `0.63.0` uses this exact native projector for controller-addressed terminals; `0.65.0` forwards successful positions immediately through original `0x1402f0c90`, while mesh misses deactivate the pointer. |
...
| `0x1400bcd90` | Confirmed, control hook built | Shared AngelScript `iLuxEntity.SetMatrix` registration target used by derived Lux entity types. It owns exact independent `HudObject`, socketed `*_HudObject` classification, `PlayerHands_*`, and `Flashlight` policies. During exact Read state `10`, `0.65.1` preserves native translation/timing, applies configured apparent scale, and after grip engagement persists unrestricted controller-relative pitch/yaw/roll. Destruction evicts pose state; every failed identity, state, distance, tracking, or matrix gate forwards the original pointer. Ghidra: `SOMA_iLuxEntity_SetMatrix`. |
| `0x1400be8f0` | Confirmed, documented | Native `cLuxProp::AttachToSocket` implementation registered under the exact five-argument AngelScript method (plus native `this`). It owns attachment state at child `+0x740`, resolves a named socket from the parent mesh, stores the socket at attachment `+0xa0`, and preserves snap/rotation/lock policy. The apartment medicine script attaches `Tracer_Fluid_HudObject` to `PlayerHands_0` socket `R_Hand`; SOMAVR must preserve this native prop ownership while driving wrists/arms. Ghidra: `SOMA_cLuxProp_AttachToSocket`. |
...
| `0x140238750` | Confirmed, control hook built | `HPL3_PidControllerVec3_Output`. Grab force (`400/0/40`) receives unbounded initial selected-hit-to-grip correction plus separately bounded post-acquisition controller travel; Grab torque (`40/0/0.4|0.1`) receives absolute tracked-hand target minus measured body velocity with complete error bounded at the native `6 rad/s` response cap. The 0.68.2 live log proved the attempted semantic Slide readiness gate deferred every event with null `player+0xc8`, so 0.70 packages direct Slide `6/0/0.1`: controller velocity plus displacement error projected onto selected body joint-0 pin. SwingDoor `10/0/1` uses hand point velocity around the hinge without wrist twist; Lever retains twist. SOMA owns integration, limits, mass, collision, joints, gravity, sounds, and callbacks; unrelated tuples pass through. |
...
| `0x1404a9a40` | Confirmed, consumed | `iPhysicsBody::GetLocalMatrix()` accessor returning body transform at `+0x50`. `0.65.1` normalizes the basis for current and anchor Grab-body orientation. Ghidra: `HPL3_PhysicsBody_GetLocalMatrix`. |
| `0x1401b0710` | Confirmed, consumed | `iPhysicsBody::GetAngularVelocity()` virtual thunk: `MOV RAX,[RCX]; JMP [RAX+0x90]`, with the `cVector3f` hidden return buffer forwarded in `RDX`. `0.65.1` subtracts this measured velocity from the absolute controller-relative target before the native Grab torque PID. Ghidra: `HPL3_PhysicsBody_GetAngularVelocity`. |
...
| `0x1401822e0` | Confirmed, consumed | Registered `iPhysicsJoint::GetPivotPoint()` leaf; returns `joint+0xf4`. `0.62.0` combines this native hinge pivot with `GetPinDir()` for signed translation-around-pivot velocity; `0.65.0` adds controller wrist angular velocity projected directly onto the same pin. Ghidra: `HPL3_PhysicsJoint_GetPivotPoint`. |
| `0x14049c720` | Confirmed, hardened control patch built | `HPL3_Script_iPhysicsBody_AddImpulse`. Nine-byte virtual thunk dispatching through body vtable `+0x130`; the lane requires exactly one matching executable-section signature for the thunk plus three INT3 bytes, suspends all peer process threads, rejects any RIP inside the 12-byte patch window, and revalidates expected bytes before both installation and restoration. It redirects only a 350 ms controller-armed Grab throw before calling the concrete virtual method. The signature is authority; this RVA is evidence for the supported executable build. |
...
| `0x1400ccc90` | Confirmed, control built | Registered `cLux_GetGamePaused()` wrapper. Its verified RIP-relative load identifies the game-context slot; the wrapper then reads subsystem `gameContext+0xc8` and paused byte `+0x2d4`. Since the subsystem is null during early startup, `0.78.1` no longer calls the wrapper: it validates and reads each owner directly, failing closed until the chain exists. Used by locomotion, input/menu/HUD ownership, and retained hands. Ghidra: `SOMA_GetGamePaused`. |
...
| `0x14015ba20` | Confirmed, native-phase control hook built | Per-frame player-helper update. `self` is simultaneously the live analog owner; root player identity is `self-0x110`. Version 0.71 consumes queued Normal Move and physical-manipulation Look at entry, validates state against the root, checks state context at helper `+0xc8 -> +0x10`, and calls `0x140154fb0` with `self`. Ghidra: `SOMA_cLuxPlayerHelper_Update`. |
| `0x14023b110` | Confirmed, documented | `cCharacterBody::SetCamera`. Writes camera owner `+0x1b0`; distinct from viewport camera selection. Ghidra: `HPL3_CharacterBody_SetCamera`. |
| `0x140071f10` | Confirmed, documented | `cViewport::SetCamera`; writes viewport camera `+0x18`. Ghidra: `HPL3_Viewport_SetCamera`. |
| `0x140270d70` | Confirmed, documented | `cCamera::SetRotateMode`; writes camera `+0x6c`. Matrix mode is a structural authored-camera signal. Ghidra: `HPL3_Camera_SetRotateMode`. |
| `0x140270ee0` | Confirmed, documented | Copies authored rotation matrix to camera `+0x1f4`. Ghidra: `HPL3_Camera_SetRotationMatrix`. |
| `0x140165270` | Confirmed, passive probe built | `iLuxEntity::GetMeshEntity` virtual wrapper through vtable `+0xd8`, called only for exact `PlayerHands_*`. Ghidra: `SOMA_iLuxEntity_GetMeshEntity`. |
| `0x1401fdc30` | Confirmed, documented | Indexed bone-state accessor over mesh-entity array `+0x2a0`. Ghidra: `HPL3_MeshEntity_GetBoneState`. |
| `0x140200980` | Confirmed, passive probe built | Resolves bone state by native string. Ghidra: `HPL3_MeshEntity_GetBoneStateFromName`. |
| `0x140200420` | Confirmed, documented | Resolves mesh socket by native string. Ghidra: `HPL3_MeshEntity_GetSocket`. |
| `0x14023f600` / `0x14023f820` | Confirmed, passive reads built | Node world matrix/position; world matrix stored at `+0x84`. Ghidra: `HPL3_Node3D_GetWorldMatrix`, `HPL3_Node3D_GetWorldPosition`. |
| `0x14023f610` | Confirmed, passive read built | `cNode3D::GetParent` returns node `+0x180`. Version 0.75 compares each wrist parent to the named `j_L_Arm_10` / `j_R_Arm_10` bone and uses its world matrix only for a non-mutating post-transform candidate. Ghidra: `HPL3_Node3D_GetParent`. |
| `0x14051a310` | Confirmed, passive read built | `cNode3D::GetLocalMatrix` returns node `+0x44`. Version 0.75 combines this animated local matrix with parent world and a desired world position to validate post-transform composition without writing the node. Ghidra: `HPL3_Node3D_GetLocalMatrix`. |
| `0x14023fee0` | Confirmed, guarded control built | `cNode3D::SetMatrix`; copies a supplied local matrix to node `+0x44` and updates world/descendants according to the boolean argument. Version 0.80 restores cached arm-chain locals before each IK solve. Live 0.82 Frida evidence proved both clavicles share parent `j_Root`, which medicine moves by local Y `+0.6643875`; version 0.83 restores this validated shared parent once per frame before either 34-node side chain. Ghidra: `HPL3_Node3D_SetMatrix`. |
| `0x1404a9470` / `0x1404a9480` | Confirmed, documented | Return pre/post animation matrices at `+0xc8/+0x108`. Ghidra: `HPL3_Node3D_GetPreAnimTransform`, `HPL3_Node3D_GetPostAnimTransform`. |
| `0x140031c90` / `0x1404a9490` | Confirmed, guarded control built | Enable pre/post animation transforms at `+0xc4/+0xc5`. Version 0.76 enables wrist post ownership only for one synchronous apply call, then restores the native byte. Ghidra: `HPL3_Node3D_SetUsePreTransform`, `HPL3_Node3D_SetUsePostTransform`. |
| `0x140031ca0` / `0x1404a94a0` | Confirmed, guarded control built | Set pre/post animation matrices. Version 0.76 temporarily installs each independent wrist candidate and immediately restores/verifies the native 64 bytes. Ghidra: `HPL3_Node3D_SetPreTransform`, `HPL3_Node3D_SetPostTransform`. |
| `0x1402401d0` / `0x140240290` | Confirmed, guarded control built | Apply pre/post transforms. HPL2 and Ghidra confirm post is left-multiplied into local only while `UsePostTransform` is true. Clearing the flag and calling again is a no-op, not restoration; 0.80 restores local through `0x14023fee0`. Ghidra: `HPL3_Node3D_ApplyPreAnimTransform`, `HPL3_Node3D_ApplyPostAnimTransform`. |
...
| `cLuxProp` | `+0x740` | Native attachment-state pointer created/updated by `SOMA_cLuxProp_AttachToSocket`. |
| attachment state | `+0xa0` | Resolved parent mesh socket pointer; live medicine ownership is `R_Hand`. |
