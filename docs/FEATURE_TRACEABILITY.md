...
| `FEATURE.INTERACTION_RAY` | BUILT | `HPLInteractionBridge`, `HPLInteractionMath`, `HPLCameraBridge`, `HPLInputBridge`, `HPLGrabBridge` | `0x1400cd750`, inner raycast `0x1401438c0`, distance `+0x18`, body `+0x20`, entity `+0x28`, two local hand candidates, one outer finalizer, sticky hand owner, native `CanInteract` | `ADDRESS_REGISTRY.md`, `VR_COMPATIBILITY_RE.md`, `TEST_CHECKLISTS.md` | Live-test independent left/right focus and activation, overlap arbitration, exact one-callback behavior, and initiating-hand continuity across Slide/hinge/Grab/Read/terminal |
