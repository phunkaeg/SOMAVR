...
#
# This is the tool that answers questions a window screenshot cannot. A SOMA
# window capture sees only the desktop backbuffer; XR projection and quad
# layers exist in the compositor and may never appear there. Here they are
# composited and counted.
#
...
#
# Usage:
#   $s = .\tools\xrsim\scripts\xrsim-shot.ps1 -Out "$env:TEMP\somavr\a"
[CmdletBinding()]
param(
    [string]$Out = "",
...
& $stateScript -Dir $Dir -For "captureSeq=$($capSeqBefore + 1)" -TimeoutSec $TimeoutSec -Quiet | Out-Null
