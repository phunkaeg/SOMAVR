...
| `0x140213970` | Confirmed, control hook built | Renders one `cGuiSet`; 2D sets draw into the current framebuffer. `0.31.0` accumulates exact gameplay HUD draws into a transparent capture. `0.67.0` additionally preserves the native state-8 physical terminal render, then replays only the exact focused flat terminal set into the additive OpenXR HUD capture. Ghidra: `HPL3_GuiSet_Render`. |
