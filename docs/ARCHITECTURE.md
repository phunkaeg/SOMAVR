...
| `HPLComfortBridge` | Transactional guarded ownership of semantic camera-add, Set/Fade camera-roll, and world DoF boundaries during active VR | Lower-level camera transforms, player-state ownership, fades/tone mapping, or broad post chains |
| `HPLComfortMath` | Pure camera-add/roll enum classification, independent suppression policy, and high-motion player-state transition classification | Native pointers, hook lifecycle, tracking state, or logging |
