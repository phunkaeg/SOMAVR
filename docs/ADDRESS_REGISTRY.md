...
| `0x1400cd750` | Confirmed, control hook built | Registered global `GetClosestEntity` wrapper used by `Utility_PickBasics`. In addition to controller start/direction substitution, `0.19.0` reads finalized `cLuxClosestEntityData` entity `+0x18`, body `+0x20`, and distance `+0x28` into a validated world-hit snapshot. Native length, LOS, `CanInteract`, focus, and callbacks remain owned by SOMA. Ghidra: `SOMA_GetClosestEntity`. |
