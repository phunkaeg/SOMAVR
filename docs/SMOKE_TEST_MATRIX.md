...
| S11 | Tracking/runtime interruption | Remove HMD/controller tracking, change runtime focus, then restore | Pose-age hold/expiry, zero-layer path, release of held inputs, recovery blackout, correct per-hand interaction-profile rows | Stale pose/profile, stuck input, visible invalid frame, session restart loop, or unresolved active controller profile |
