...
| `FEATURE.PHYSICAL_MANIPULATION` | BUILT | `HPLInputBridge`, `HPLInputMath`, `OpenXRInput` | shipped states Wheel `3`, Slide `4`, SwingDoor `5`, Lever `6`, Tear `7`, Read `10`; `RotateBase::OnAnalogInput -> mvMoveAdd`; relative grip/head pose; native right-click cancel | `FUTURE_SYSTEMS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test 3x Slide scaling, unchanged other physical states, grip-held Read rotation, Secondary cancel, tracking reacquisition, and native constraints/callbacks |
...
| `FEATURE.TELEMETRY` | PROVEN | `OpenGLHooks`, `OpenGLMatrixAnalysis`, `HPLCompatibilityProbe`, `HPLPlayerState`, `HPLInputBridge`, logger, config | Swap/FBO/matrix/runtime/stage/audio/player-state/post-effect bounded logs, camera ownership handoff counters | all current docs | Correlate authored-camera handoffs, stage-tagged draws, GUI state, and per-effect identities in a live run |
