...
| `FEATURE.PHYSICAL_MANIPULATION` | BUILT | `HPLGrabBridge`, `HPLInputBridge`, `HPLInputMath`, `HPLInteractionBridge`, `OpenXRInput` | states Wheel `3`, Slide `4`, Door `5`, Lever `6`, Tear `7`, Read `10`; Slide PID `0x140238750`, body joint vector `+0x168`, pin `+0xe8`; grip orientation; A/B native cancel | `FUTURE_SYSTEMS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test camera-independent drawer/curtain joint motion, smooth Read orientation, A cancel, no special-state turn/blackout, and unchanged native limits/sounds/callbacks |
