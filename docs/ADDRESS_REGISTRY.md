...
| `0x1400cd710` | Confirmed, guarded control built | Registered AngelScript `CheckLineOfSight` wrapper. ABI is `bool(const float* start, const float* end, bool shadowOnly, bool staticOnly)`; supplies a null skip entity and calls `0x140143650`. `0.24.0` signature-guards it for static-world room-scale translation safety. Ghidra: `SOMA_CheckLineOfSight`. |
...
| `0x1400cca90` | Confirmed, passive probe built | `SOMA_GetGameHudImGui`; follows game-context `+0xe8`, then `+0x160`. Registered by exact `cImGui@ GetGameHudImGui()` string. `0.22.0` signature-guards all 22 bytes. |
| `0x140071f20` | Confirmed, passive probe built | `HPL3_ImGui_GetSet`; five-byte registered wrapper returns `cImGui +0x18` as the owned `cGuiSet`. `0.22.0` uses it only for render identity telemetry. |
| `0x1400cd750` | Confirmed, control/result hook built | Registered global `GetClosestEntity` wrapper used by `Utility_PickBasics`. Controller start/direction substitution and finalized `cLuxClosestEntityData` entity `+0x18`, body `+0x20`, and distance `+0x28` feed the validated world-hit snapshot; `0.20.0` consumes that exact result for the OpenXR depth reticle and focus-change haptic. Native length, LOS, `CanInteract`, focus, and callbacks remain owned by SOMA. Ghidra: `SOMA_GetClosestEntity`. |
| `0x140484ea0` | Confirmed, observer hook built | Registered `cScript_RunGlobalFunc` thunk. `0.21.0` signature-guards all 21 bytes and observes only exact native strings `LuxPlayer`, empty class, and `_Global_SetCrosshairState`; original callback execution remains authoritative. Ghidra: `HPL3_Script_RunGlobalFunc`. |
...
| `0x1400bcd90` | Confirmed, control hook built | Shared AngelScript `iLuxEntity.SetMatrix` registration target used by derived Lux entity types. `0.16.0` replaces exact normal quarter-scale `PlayerHands_*` roots from tracked grip; `0.23.0` additionally replaces only exact `Flashlight` matrices from fresh dominant aim. Every failed gate forwards the original pointer. Ghidra: `SOMA_iLuxEntity_SetMatrix`. |
| `0x14000fb60` | Confirmed, identity gate built | Compact inherited `iLuxEntity.GetName` accessor registered for `cLuxProp`; returns native `tString` at entity `+0x120`. Exact `PlayerHands_*` prefix and exact `Flashlight` matching bound all transform telemetry/control. Ghidra: `SOMA_iLuxEntity_GetName`. |
...
| `0x140238750` | Confirmed, control hook built | `HPL3_PidControllerVec3_Output`. `0.18.0` changes only exact Grab-state force (`400/0/40`) and torque (`40/0/0.4|0.1`) PID errors. Controller translation and shortest-arc rotation targets are opt-in; every other call passes through. |
