// somavr_xrsim64: session lifecycle, the session state machine, reference and
// action spaces, and swapchains.
//
// There is exactly one session at a time, which is all the mod or somavr_xrsim_smoke
...
void session_focus_lose(uint32_t holdMs) {
    ...
    // explicit `focus regain` ever came back - unnoticed while the tests used
    // only explicit regain commands.
    {
        std::lock_guard<std::mutex> lock(g_mutex);
...
static XrResult impl_CreateSession(XrInstance instance, const XrSessionCreateInfo* info,
    ...
    const XrGraphicsBindingOpenGLWin32KHR* binding = nullptr;
    for (const auto* next = static_cast<const XrBaseInStructure*>(info->next); next;
        ...
        if (next->type == XR_TYPE_GRAPHICS_BINDING_OPENGL_WIN32_KHR)
            binding = reinterpret_cast<const XrGraphicsBindingOpenGLWin32KHR*>(next);
    }
    if (!binding || !binding->hDC || !binding->hGLRC) return XR_ERROR_GRAPHICS_DEVICE_INVALID;
    ...
    // The runtime never takes ownership of the application's WGL objects. It
    // records them so swapchain creation and capture can fail closed if called
    // without the expected context current.
    g_deviceContext = binding->hDC;
    g_glContext = binding->hGLRC;
    ...
    *out = make_session_handle();
    ...
    XRSIM_LOG("xrsim: session created on app WGL context hdc=%p hglrc=%p",
              static_cast<void*>(g_deviceContext), static_cast<void*>(g_glContext));
    return XR_SUCCESS;
}
...
void session_destroy_all() {
    ...
    const bool canDeleteTextures = g_glContext && wglGetCurrentContext() == g_glContext;
    for (auto& sc : g_swapchains) {
        if (canDeleteTextures && sc.imageCount > 0) glDeleteTextures(sc.imageCount, sc.images);
        sc = SimSwapchain{};
    }
...
bool space_pose(const SimSpace& space, const FrameSnapshot& snap, Pose& out, bool& tracked) {
    ...
    if (space.isAction) {
        ...
        const bool isAim = act && (act->controls[h] == VC_POSE_AIM_L
                                   || act->controls[h] == VC_POSE_AIM_R);
...
static XrResult impl_EnumerateSwapchainFormats(XrSession session, uint32_t capacity,
    ...
    if (!session_valid(session)) return XR_ERROR_HANDLE_INVALID;
    if (!countOutput) return XR_ERROR_VALIDATION_FAILURE;
    // OpenGL internal-format tokens are the native OpenXR swapchain format IDs.
    // SOMAVR prefers sRGB and then RGBA8, so preserve that real-runtime order.
...
static XrResult impl_CreateSwapchain(XrSession session, const XrSwapchainCreateInfo* info,
    ...
    glGenTextures(sc.imageCount, sc.images);
    for (uint32_t i = 0; i < sc.imageCount; ++i) {
        glBindTexture(GL_TEXTURE_2D, sc.images[i]);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
        glTexImage2D(GL_TEXTURE_2D, 0, static_cast<GLint>(info->format),
                     static_cast<GLsizei>(info->width), static_cast<GLsizei>(info->height), 0,
                     GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
        if (sc.images[i] == 0 || glGetError() != GL_NO_ERROR) {
            glDeleteTextures(sc.imageCount, sc.images);
            glBindTexture(GL_TEXTURE_2D, static_cast<GLuint>(previousTexture));
            sc = SimSwapchain{};
            sc.gen = gen;
            XRSIM_LOG("xrsim: glTexImage2D failed for a %ux%u fmt 0x%llx swapchain",
...
static XrResult impl_EnumerateSwapchainImages(XrSwapchain handle, uint32_t capacity,
    ...
    *countOutput = sc->imageCount;
    ...
    for (uint32_t i = 0; i < sc->imageCount; ++i) {
        if (gl[i].type != XR_TYPE_SWAPCHAIN_IMAGE_OPENGL_KHR) return XR_ERROR_VALIDATION_FAILURE;
        gl[i].image = sc->images[i];
    }
    return XR_SUCCESS;
}
...
GLuint swapchain_last_image(XrSwapchain handle, uint32_t* outW, uint32_t* outH,
                            int64_t* outFormat) {
    std::lock_guard<std::mutex> lock(g_mutex);
    SimSwapchain* sc = swapchain_get(handle);
    if (!sc || !sc->everReleased) return 0;
    if (outW) *outW = sc->width;
    if (outH) *outH = sc->height;
    if (outFormat) *outFormat = sc->format;
