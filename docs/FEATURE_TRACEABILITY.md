...
| `FEATURE.PER_EYE_VIEW_HISTORY` | BUILT | `HPLPerEyeViewHistory`, `HPLPerEyeViewHistoryMath`, `HPLCameraBridge`, `HPLCompatibilityProbe` | `0x1401f1480`, active frustum view `+0x158`, renderer history pointer `+0x438`, previous view `+0x80`, pending/actual eye-pose transaction | `ADDRESS_REGISTRY.md`, `VR_COMPATIBILITY_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test history status, one-time seed, alternating restore/capture, scene transitions, temporal image stability, performance, and hard rollback; continue RE for exposure, projection, velocity, and post-effect histories |
