...
| `0x1402cb5a0` | Confirmed, guarded control hook built | **`iEntity3D::SetVisible(bool)` --- takes an `Entity3D`, NOT a `cMeshEntity`.** Writes `+0x38`, dispatches vtable slot `+0xd0`, then dereferences `+8`. Requires a 27+ slot vtable and a valid-or-null pointer at `+8`. Passing a `cMeshEntity` (14 slots) or an `iLuxEntity` crashes --- see [CRASH_F10_DUAL_RENDER_2026-09-01.md](CRASH_F10_DUAL_RENDER_2026-09-01.md). The SOMAVR-side name `kMeshEntitySetVisibleRva` is a **misnomer** kept for source compatibility; it is not the mesh-entity entry point. Ghidra: `HPL3_Entity3D_SetVisible`. |
| `0x1401fe390` | Confirmed by decompilation, not yet hooked | **`cMeshEntity::SetVisible(bool)` --- the correct entry point for a `cMeshEntity*`** (the pointer `GetMeshEntity` returns). Guards on `+0x168`, then iterates the child vector `[+0x1f0, +0x1f8)` and calls `0x1402cb5a0` on each child, which are the real `Entity3D` objects. Idempotent, so calling it when already in the requested state is free. Use this lane, signature-verified separately, for any mesh-level visibility work. |
