...
| `FEATURE.XR_FOCUS_PACING` | BUILT | `OpenXRRuntime`, `OpenXRFramePacingMath` | session-state events, ever-focused latch, bounded `xrWaitFrame` telemetry, frame-open invariant, stale-cache invalidation on focus recovery, symmetric F10 session/instance teardown | `BIOSHOCK_VR_TRANSFER_AUDIT.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Prove VISIBLE/unfocused recovery, then F10 stop/restart; active-VR `xrWaitFrame` still runs on `SwapBuffers`, so measure direct cadence/backpressure before any worker-thread claim |
...
| `FEATURE.TRACKING_RESILIENCE` | BUILT | `OpenXRRuntime`, `HPLCameraBridge` | one upcoming-render locate per tick, immutable AFR pair pose/base with 100 ms guard, retained/black projection, recovery blackout | `BUILD_HISTORY.md`, `CURRENT_STATE.md`, `TEST_CHECKLISTS.md` | Live-test brief and extended HMD tracking loss without mixed-pair state, stale-eye corruption, stereo teardown, visible recovery flash, or any zero-layer submit |
