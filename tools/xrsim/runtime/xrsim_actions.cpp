// somavr_xrsim64: action sets, actions, interaction profiles, and action state.
//
...
const BindingEntry kTouch[] = {
    ...
    {"/user/hand/left/input/trigger/click", VC_TRIGGER_L, 0},
    {"/user/hand/right/input/trigger/click", VC_TRIGGER_R, 1},
...
bool control_bool(const Rig& rig, VirtualControl c) {
    ...
    case VC_TRIGGER_L: return rig.trigger[0] >= 0.5f;
    case VC_TRIGGER_R: return rig.trigger[1] >= 0.5f;
    case VC_SQUEEZE_L: return rig.squeeze[0] >= 0.5f;
    case VC_SQUEEZE_R: return rig.squeeze[1] >= 0.5f;
    default: return false;
    }
}

int subaction_hand(XrPath path, const SimAction& action) {
    if (path != XR_NULL_PATH) {
        const char* text = path_str(path);
        if (text && strstr(text, "/user/hand/left") != nullptr) return 0;
        if (text && strstr(text, "/user/hand/right") != nullptr) return 1;
    }
    if (action.controls[0] != VC_NONE && action.controls[1] == VC_NONE) return 0;
    if (action.controls[1] != VC_NONE && action.controls[0] == VC_NONE) return 1;
    return 0;
}
...
void actions_reset_session() {
    ...
    for (auto& a : g_actions) {
        if (!a.used) continue;
        for (int h = 0; h < 2; ++h) {
            a.syncInitialized[h] = false;
            a.syncedActive[h] = false;
            a.changed[h] = false;
        }
    }
}
...
static XrResult impl_SuggestInteractionProfileBindings(
    ...
    for (uint32_t i = 0; i < info->countSuggestedBindings; ++i) {
        ...
        // profile's suggestion may overwrite a resolved control for that hand.
        if (pe->authoritative || a->controls[e->hand] == VC_NONE) {
            a->controls[e->hand] = e->control;
        }
...
static XrResult impl_SyncActions(XrSession session, const XrActionsSyncInfo*) noexcept {
    ...
    for (SimAction& action : g_actions) {
        ...
        for (int h = 0; h < 2; ++h) {
            const VirtualControl control = action.controls[h];
            const bool active = control != VC_NONE
                && (!control_is_pose(control) || rig.handValid[h]);
            bool valueChanged = action.syncInitialized[h]
                && active != action.syncedActive[h];

            if (action.type == XR_ACTION_TYPE_BOOLEAN_INPUT) {
                const bool value = active && control_bool(rig, control);
                valueChanged = valueChanged
                    || (action.syncInitialized[h] && value != action.syncedBool[h]);
                action.syncedBool[h] = value;
            } else if (action.type == XR_ACTION_TYPE_FLOAT_INPUT) {
                const float value = active ? control_float(rig, control) : 0.0f;
                valueChanged = valueChanged
                    || (action.syncInitialized[h] && value != action.syncedFloat[h]);
                action.syncedFloat[h] = value;
            } else if (action.type == XR_ACTION_TYPE_VECTOR2F_INPUT) {
                XrVector2f value{};
                if (active && (control == VC_STICK_L || control == VC_STICK_R)) {
                    value.x = rig.stick[h][0];
                    value.y = rig.stick[h][1];
                }
                valueChanged = valueChanged || (action.syncInitialized[h]
                    && (value.x != action.syncedVector[h].x
                        || value.y != action.syncedVector[h].y));
                action.syncedVector[h] = value;
            }

            action.changed[h] = valueChanged;
            action.syncedActive[h] = active;
            action.syncInitialized[h] = true;
            if (valueChanged) action.lastChangeTime[h] = snap.displayTime;
        }
    }
    return XR_SUCCESS;
}
...
static XrResult impl_GetActionStateBoolean(XrSession session, const XrActionStateGetInfo* info,
    ...
    const int hand = subaction_hand(info->subactionPath, *a);
    const bool live = inputs_live() && a->syncedActive[hand];
    state->type = XR_TYPE_ACTION_STATE_BOOLEAN;
    state->isActive = live ? XR_TRUE : XR_FALSE;
    state->currentState = live && a->syncedBool[hand] ? XR_TRUE : XR_FALSE;
    state->changedSinceLastSync = a->changed[hand] ? XR_TRUE : XR_FALSE;
    state->lastChangeTime = a->lastChangeTime[hand];
    return XR_SUCCESS;
}
...
static XrResult impl_GetActionStateFloat(XrSession session, const XrActionStateGetInfo* info,
    ...
    state->isActive = live ? XR_TRUE : XR_FALSE;
    state->currentState = live ? a->syncedFloat[hand] : 0.0f;
    state->changedSinceLastSync = a->changed[hand] ? XR_TRUE : XR_FALSE;
    state->lastChangeTime = a->lastChangeTime[hand];
    return XR_SUCCESS;
}
...
static XrResult impl_GetActionStateVector2f(XrSession session, const XrActionStateGetInfo* info,
    ...
    state->currentState = live ? a->syncedVector[hand] : XrVector2f{};
    state->changedSinceLastSync = a->changed[hand] ? XR_TRUE : XR_FALSE;
    state->lastChangeTime = a->lastChangeTime[hand];
    return XR_SUCCESS;
}
...
static XrResult impl_GetActionStatePose(XrSession session, const XrActionStateGetInfo* info,
    ...
    const int hand = subaction_hand(info->subactionPath, *a);
    const bool live = inputs_live() && a->syncedActive[hand]
        && control_is_pose(a->controls[hand]);
