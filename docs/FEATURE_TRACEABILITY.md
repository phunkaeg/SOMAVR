...
| `FEATURE.TERMINAL_OVERLAY` | BUILT | `HPLHudBridge`, `HPLTerminalBridge`, `HPLInputBridge`, `OpenXRGLBridge`, `OpenXRRuntime` | state `8`, manager world owner `+0x170`, focused set `+0x180/+0x18`, flat `1024x577` FBO, one `0x140213970` render plus framebuffer blit | `FUTURE_SYSTEMS_RE.md`, `ADDRESS_REGISTRY.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test coherent nonflashing copy, continuous two-hand pointer alignment, native widget clicks, physical-screen preservation, alpha/order, exit cleanup, and rollback |
...
| `FEATURE.PHYSICAL_MANIPULATION` | BUILT | `HPLGrabBridge`, `HPLGrabMath`, `HPLReadMath`, `HPLInputBridge`, `HPLInputMath`, `HPLInteractionBridge`, `OpenXRInput` | Wheel `3`, Slide `4`, Door `5`, Lever `6`, Tear `7`, Read `10`, MovingButton `13`; queued Look delivered at guarded player-helper phase `0x14015ba20` through `0x140154fb0`; native Read translation plus full-axis orientation | `FUTURE_SYSTEMS_RE.md`, `ADDRESS_REGISTRY.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Prove native-phase dispatch counters rise and curtain/drawer/tap respond without faults; verify Read entrance settles once at native distance with `2x` apparent scale |
