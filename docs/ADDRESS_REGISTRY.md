...
| `0x140154fb0` | Confirmed, direct control bridge built | `cLuxPlayer::OnAnalogInput(int,const cVector3f&)`. Dispatches analog input to the current state script at player `+0xc8`, secondary handler `+0xf0`, then player-level handling. `0.68.0` signature-guards this boundary and sends controller manipulation as native Look type `0`; Windows mouse movement is fallback only. Ghidra: `SOMA_cLuxPlayer_OnAnalogInput`. |
