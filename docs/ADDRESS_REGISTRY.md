...
| `0x140154fb0` | Confirmed, guarded control call built | `cLuxPlayer::OnAnalogInput(int,const cVector3f&)`. Dispatches to current state script at player `+0xc8`, secondary handler `+0xf0`, then player-level handling. Direct render/update-phase invocation in `0.68.0` is rejected by dump evidence. `0.68.2` calls it only from hooked native player-helper update `0x14015ba20`, after exact state and script-context readiness checks. Ghidra: `SOMA_cLuxPlayer_OnAnalogInput`. |
...
| `0x140213970` | Confirmed, control hook built | Renders one `cGuiSet`; 2D sets draw into the current framebuffer. `0.31.0` accumulates exact gameplay HUD draws into a transparent capture. `0.68.0` preserves one native state-8 terminal render and blits its completed FBO into the OpenXR HUD capture; replaying this stateful function caused fragmented/flashing content. Ghidra: `HPL3_GuiSet_Render`. |
...
| `0x140238750` | Confirmed, control hook built | `HPL3_PidControllerVec3_Output`. Grab force (`400/0/40`) receives unbounded initial selected-hit-to-grip correction plus separately bounded post-acquisition controller travel; Grab torque (`40/0/0.4|0.1`) receives absolute tracked-hand target minus measured body velocity with complete error bounded at the native `6 rad/s` response cap. Released Slide script proves normal curtains/drawers consume semantic Look through `mvMoveAdd`, so `0.67.0` packages `SlideDirectVelocity=0`; optional direct Slide `6/0/0.1` remains diagnostic. SwingDoor/Lever `10/0/1` retain their pin routes. SOMA owns integration, limits, mass, collision, joints, gravity, sounds, and callbacks; unrelated tuples pass through. |
...
| `0x14015ba20` | Confirmed, native-phase control hook built | Per-frame player helper update. Derives `cLuxPlayer` as `self-0x110` and runs collision, motion averaging, head, and camera helpers. `0.68.2` consumes queued manipulation at entry only after active state-script context readiness, then calls `0x140154fb0`. Ghidra: `SOMA_cLuxPlayerHelper_Update`. |
