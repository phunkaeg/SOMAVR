...
| `FEATURE.VIEWMODEL` | RE_REQUIRED | `HPLCameraBridge`, `HPLInputBridge`, future hands bridge | world grip pose, `PlayerHandsHandler`, `0x1400bcd90`, `R_Hand`, tool `HudObject` | `FUTURE_SYSTEMS_RE.md` | Build a bounded `PlayerHands_*` identity probe at the shared SetMatrix wrapper, then override only the default camera-follow transform without breaking animations |
