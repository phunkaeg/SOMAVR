...
| `FEATURE.XR_GL_SUBMISSION` | PROVEN | `OpenXRRuntime`, `OpenXRGLBridge`, `OpenGLOwnership` | OpenGL swapchains/FBOs, own-GL detour bypass, one prioritized projection on every begun `xrEndFrame` | `CURRENT_STATE.md`, `BUILD_HISTORY.md` | Headset-test shouldRender false, tracking loss, loading, context recovery, and format/color-space behavior without a frozen compositor or HPL telemetry contamination |
