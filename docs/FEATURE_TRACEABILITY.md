...
| `FEATURE.PHYSICAL_MANIPULATION` | BUILT | `HPLGrabBridge`, `HPLGrabMath`, `HPLReadMath`, `HPLInputBridge`, `HPLInputMath`, `HPLInteractionBridge`, `OpenXRInput` | Wheel `3`, Slide `4`, Door `5`, Lever `6`, Tear `7`, Read `10`, MovingButton `13`; guarded `0x140154fb0` native Look dispatch; current-native Read position scaling and full-axis grip orientation | `FUTURE_SYSTEMS_RE.md`, `ADDRESS_REGISTRY.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test curtain drag, tap/cupboard direction changes, Read doubled distance/cancel/camera stability; preserve native limits, physics, sounds, callbacks, and rollback |
