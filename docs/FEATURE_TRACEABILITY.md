...
| `FEATURE.XR_GL_SUBMISSION` | PROVEN | `OpenXRRuntime`, `OpenXRGLBridge`, `OpenGLOwnership` | OpenGL swapchains/FBOs, own-GL detour bypass, one prioritized projection on every begun `xrEndFrame`, per-eye acquire/wait/copy/flush/release timing and display-budget pressure | `CURRENT_STATE.md`, `BUILD_HISTORY.md`, `OPENXR_GL_TRANSFER_RE.md` | Run the controlled VirtualDesktopXR/SteamVR A/B; retain native GL unless runtime handoff cost materially justifies a D3D11 interop backend |
...
| `FEATURE.AFR_STEREO` | PROVEN | `HPLCameraBridge`, `OpenXRRuntime`, `OpenXRGLBridge` | F11, per-eye cache and submitted render pose, fill-committed phase, pair-latched rotation with per-eye position | `BUILD_HISTORY.md`, `RUNTIME_ANALYSIS_0.5.1.md` | Headset-prove fill commits alternate 0/1 through loading/tracking interruption and pair rotation remains rigid without positional-baseline collapse |
