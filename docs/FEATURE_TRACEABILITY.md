...
| `FEATURE.DUAL_RENDER` | EXPERIMENTAL | `HPLCompatibilityProbe`, `HPLDualRenderMath`, `OpenXRRuntime` | `0x140298850`, `0x140298630`, immediate first-eye cache, `Ctrl+F6` one-frame replay with GUI bit removed | `VR_COMPATIBILITY_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live replay proves same-pose opposite eyes, bounded cost, stable temporal state, and whether duplicated world/post-post callbacks are safe enough to split or repeat |
