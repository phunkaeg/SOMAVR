...
| `0x1402cb5a0` | Confirmed, guarded control hook built | Native entity/mesh `SetVisible(bool)` wrapper; writes visibility at `+0x38` and notifies native render ownership. Version 0.77 retains only the cached exact player-hands mesh. Ghidra: `HPL3_Entity3D_SetVisible`. |
