...
| `OpenXRRuntime` | Instance/system/session state, delayed loss recovery, one-wait/one-locate upcoming-render frame pacing, symmetric manual suspend/restart, extension negotiation, view snapshots, projection/quad/cylinder submission, comfort-vignette envelope/layer policy, and bounded comfort-black frames | HPL camera transforms, AFR pair ownership, or raw gameplay input semantics |
