...
| `FEATURE.INTERACTION_RAY` | EXPERIMENTAL | `HPLCameraBridge`, `HPLInputBridge`, future native interaction bridge | world aim pose, `Utility_PickBasics`, native `CanInteract` | `VR_COMPATIBILITY_RE.md`, `BUILD_HISTORY.md` | Live world-ray telemetry is rigid and correctly aimed; then feed it into native closest-entity selection without replacing range/focus policy |
...
| `FEATURE.VIEWMODEL` | RE_REQUIRED | `HPLCameraBridge`, `HPLInputBridge`, future hands bridge | world grip pose, `PlayerHandsHandler`, `R_Hand`, tool `HudObject` | `FUTURE_SYSTEMS_RE.md` | Validate the grip anchor, then locate and override only the default camera-follow hand transform without breaking animations |
