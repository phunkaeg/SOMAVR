...
| S13 | Dual-render test scenes | In F10 VR, press Ctrl+F6 once in quiet, shadowed, reflective, tone/bloom, fade, and post-heavy views | One same-pose/opposite-eye replay per press, first-eye immediate cache, GUI bit removed, bounded cost/callback counts, same-pose per-effect texture/FBO ownership rows | Crash, extra replay, simulation advance, eye/pose mismatch, ownership classification from unmatched poses, resource overflow, persistent temporal artifact, or broken AFR fallback |
