...
| `FEATURE.DIEGETIC_GUI_POINTER` | BUILT | `HPLTerminalBridge`, `HPLComfortBridge`, `HPLInputBridge`, `HPLPlayerState` | wall/handheld terminal states `8/9`, `0x1403132d0`, `0x1402f0c90`, `0x140237920`, `0x1401562e0`, `0x140159360`, focused wrapper/entity `+0x180/+0x28` | `ADDRESS_REGISTRY.md`, `FUTURE_SYSTEMS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test state 8 without teleport/rotation/camera pull, physical lean, mesh-ray alignment/miss behavior/click/exit, state 9 preservation, and all three rollback levels |
...
| `FEATURE.INTERACTION_RAY` | BUILT | `HPLInteractionBridge`, `HPLCameraBridge`, `HPLInputBridge` | `0x1400cd750`, distance `+0x18`, body `+0x20`, entity `+0x28`, world aim/hit snapshots, native `CanInteract` | `COMFORT_AND_FOCUS_RE.md`, `VR_COMPATIBILITY_RE.md`, `TEST_CHECKLISTS.md` | Live-test corrected positive distance/world hit and semantic reticle across targets/states; verify no-hit/fallback clearing and selected-body continuity into Slide |
| `FEATURE.INTERACTION_RETICLE` | BUILT | `HPLInteractionBridge`, `HPLCrosshairBridge`, `HPLHudMath`, `HPLInputBridge`, `OpenXRRuntime`, `OpenXRGLBridge` | native pick aim/distance, dominant aim fallback guide, `0x140484ea0` script dispatch, exact 35-state enum, 34 shipped TGA icons, application-space alpha quads | `COMFORT_AND_FOCUS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test three-point guide identity/convergence/layer limits and its automatic yield to decoded native semantic hits; then determine whether geometry occlusion is needed |
...
| `FEATURE.PHYSICAL_MANIPULATION` | BUILT | `HPLGrabBridge`, `HPLGrabMath`, `HPLInputBridge`, `HPLInputMath`, `HPLInteractionBridge`, `OpenXRInput` | states Wheel `3`, Slide `4`, Door `5`, Lever `6`, Tear `7`, Read `10`; PID `0x140238750`, body joint vector `+0x168`, pin `+0xe8`, pivot `+0xf4`; magnitude-preserving controller velocity; grip orientation; A/B native cancel | `FUTURE_SYSTEMS_RE.md`, `ADDRESS_REGISTRY.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test speed-sensitive drawer/curtain motion and camera-independent controller-arc Door/Lever motion; preserve native limits, sounds, callbacks, and rollback; Wheel/Tear still require a direct native route |
