...
| `Soma_NoSteam.exe+0x0cd750` | Control and result bridge built | Dominant-controller native interaction ray and focus result | Replace only start/direction; `0.19.0` also publishes confirmed entity/body/distance/world hit for a future reticle while preserving native policy and callbacks. |
