...
| `FEATURE.NATIVE_STEREO` | DESIGNED | future `HPLNativeStereo`, `HPLDualRenderControl`, `HPLCompatibilityProbe`, `OpenGLHooks` | viewport `0x140298850`, render-list `0x1402975e0`, culling `0x140327570`, occlusion submit `0x1402b3c30`, pooled GL query IDs, partial refraction scene-color copies, translucent camera UBO, render-section boundary `0x140298630`, presentation `0x1402a0590` | `NATIVE_STEREO_FEASIBILITY.md`, `ADDRESS_REGISTRY.md`, `VR_COMPATIBILITY_RE.md`, `HPL_OPENGL_NOTES.md` | Use the configured continuous replay to capture first/replay query and framebuffer-copy evidence plus CPU/GPU budget, culling, shadows/reflections, temporal ownership, HUD correctness, and immediate AFR rollback before designing the world-only lane |
...
| `FEATURE.VISIBLE_HANDS` | BUILT | `HPLHandsBridge`, `HPLHandsMath`, `HPLArmIKMath`, `HPLInputBridge`, `HPLUserModuleBridge` | exact `PlayerHands_*` identity, validated shared `j_Root` plus bilateral 34-node restore, HMD-position anchor, virtual torso yaw, geometric palm calibration, reach-gated clavicles, ergonomic elbow swivel, native-seeded wake/retention, read-only module-18/script-object owner discovery at `0x140154c40`, physical states `0..8`, Read `10`, and MovingButton `13` | `AUTHORED_STATES_AND_VISIBLE_HANDS_RE.md`, `FUTURE_SYSTEMS_RE.md`, `ADDRESS_REGISTRY.md`, cross-engine `12-torso-calculations-and-ergonomics.md` | Live-prove module-owner acquisition and a script-owned safe invocation phase, then route public `PlayerHands_SetVisible(true)` so the handler creates the campaign-selected model on maps with no native seed |
