...
| `0x1402f0c90` | Confirmed, control hook and direct dispatch built | `HPL3_ImGui_SendMouseVirtualPosition`; stores position at cImGui `+0x4f3c/+0x4f40`, relative delta at `+0x4f4c/+0x4f50`, and clears first-update flag `+0x4f72`. `0.66.0` mirrors `SOMA_ImGuiManager_UpdateInput`: terminal world input owner is manager `+0x170`, with focused wrapper `+0x180`, set `+0x18`, entity `+0x28`, and screen-input flag `+0x192==0`. Both direct dispatch and hooked native updates substitute controller coordinates for this exact owner. |
...
| `0x140238750` | Confirmed, control hook built | `HPL3_PidControllerVec3_Output`. Grab force (`400/0/40`) receives selected-hit-to-grip correction; Grab torque (`40/0/0.4|0.1`) receives absolute tracked-hand target minus measured body velocity with the complete error magnitude bounded. Slide `6/0/0.1` combines pin-projected hand velocity with grip/body displacement catch-up. SwingDoor/Lever `10/0/1` retain their pin routes. SOMA owns integration, limits, mass, collision, joints, gravity, sounds, and callbacks; unrelated tuples pass through. |
