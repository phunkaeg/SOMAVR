...
| `FEATURE.CONTROLLER_HAPTICS` | BUILT | `OpenXRInput`, `OpenXRRuntime`, `HPLInputBridge`, `HPLInteractionBridge`, `HPLGameplayHapticsBridge`, `HPLGameplayHapticsMath` | vibration output action, per-hand output paths, `0x140109b30` authored rumble, focused-session guard, segmented refresh/stop envelope | `FUTURE_SYSTEMS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test damage, death, datamining, locked interaction, sustained effects, and physical-gamepad coexistence; raw unscripted contact/material events remain |
...
| `FEATURE.TONE_MAPPING_FRAME_OWNER` | BUILT | `HPLToneMappingFrame`, `HPLToneMappingFrameMath`, `HPLCompatibilityProbe` | `0x1402842d0`, `0x140284fd0`, ToneMapping `+0x8c/+0x94/+0xa0/+0xd8..+0x120`, same-pose baseline replay and single committed update | `ADDRESS_REGISTRY.md`, `FUTURE_SYSTEMS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test dark/bright transitions, scripted fades, grading changes, bloom, alternating eye order, AFR, same-frame stereo, mismatch telemetry, and config rollback |
...
| `FEATURE.HUD_LAYER` | BUILT | `HPLHudBridge`, `HPLHudMath`, `OpenXRGLBridge`, `OpenXRRuntime` | `0x1400cc9b0`, `0x1400cca90`, `0x140071f20`, `0x140213970`, same-frame additive transparent capture, VIEW-space quad or `XR_KHR_composition_layer_cylinder` | `FUTURE_SYSTEMS_RE.md`, `BUILD_HISTORY.md`, `TEST_CHECKLISTS.md` | Live-test quad/70-degree curved switching, extension fallback, GameHudSet plus GameHudImGui alpha/order, exact pause/wake/dead/inventory current-ImGui owners, and center-clear coverage |
