...
| `0x140154fb0` | Confirmed, input-phase substitution hook built | `cLuxPlayer::OnAnalogInput(int,const cVector3f&)`. Dispatches to current state script at player `+0xc8`, secondary handler `+0xf0`, then player-level handling. Direct render/update-phase invocation in `0.68.0` is rejected: dump `Soma_NoSteam.exe.111764.dmp` proves the state script can be visible before its engine context is prepared. `0.68.1` queues controller Look deltas and substitutes them only when SOMA invokes this guarded function from its native input phase. Ghidra: `SOMA_cLuxPlayer_OnAnalogInput`. |
